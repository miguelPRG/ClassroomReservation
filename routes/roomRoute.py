...
async def list_rooms(room_id: str=None):
    rooms = []
    async for room in database.sala_collection.find(filter={"_id": ObjectId(room_id)} if room_id else {} ):
...
async def update_room(room_id: str, room: RoomUpdate):
    ...
    updated_room = await database.sala_collection.find_one({"_id": ObjectId(room_id)})
