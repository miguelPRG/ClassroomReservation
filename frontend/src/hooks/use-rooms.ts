...
export function useRoomUpdate() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof ApiError ? error.message : "Não foi possível atualizar a sala."
      toast.error(message)
    },
  })
}
...
export function useRoomDelete() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message = error instanceof ApiError ? error.message : "Não foi possível deletar a sala."
      toast.error(message)
    },
  })
}